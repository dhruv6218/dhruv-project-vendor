...
export default function BulkUpload() {
  ...
  const startJob = async () => {
    ...
    try {
      ...
      setJobs((cur) => [{ id, status: "processing" }, ...cur]);
      // Simulate progress and completion
      if (progressRef.current) clearTimeout(progressRef.current);
      progressRef.current = setTimeout(() => {
        setJobs((cur) => cur.map(j => j.id === id ? { ...j, status: "completed", result_url: "/example-report.csv" } : j));
        setMsg("Job completed. You can download the results.");
      }, 2000);
    } catch (err) {
