...
export default function VerifyBank() {
  ...
  const [rows, setRows] = useState<Row[]>([
    { account: "XXXXXX7890", holder: "Apex Supplies", ifsc: "HDFC0001234", status: "Match", risk: 8 },
  ]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setRows((cur) => [{ account: account ? `XXXXXX${account.slice(-4)}` : "—", holder: holder || "—", ifsc: ifsc.toUpperCase() || "—", status: "Verified", risk: 7 }, ...cur]);
      setMsg("Bank verification simulated successfully.");
    } finally {
      setLoading(false);
    }
  };
