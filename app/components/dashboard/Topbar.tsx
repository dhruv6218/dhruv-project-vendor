...
export default function Topbar({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  useEffect(() => {
    // Simulate loading notifications count locally
    const id = setTimeout(() => setUnread(3), 300);
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-1 w-48 rounded-md border border-neutral-200/70 bg-white/90 backdrop-blur shadow hidden group-hover:block">
            <ul className="py-1 text-sm">
              ...
              <li><Link href="/dashboard/settings" className="block px-3 py-2 hover:bg-neutral-100">Profile</Link></li>
              <li><button className="w-full text-left px-3 py-2 hover:bg-neutral-100">Sign out</button></li>
            </ul>
          </div>
        </div>
      </div>
